...
setGeneric("fitPoisthNorm",
           signature = c("object"),
           function(object, ...)
             standardGeneric("fitPoisthNorm"))

#' @rdname fitPoisthNorm-methods
#' @aliases fitPoisthNorm,NanoStringGeoMxSet-method
setMethod("fitPoisthNorm", "NanoStringGeoMxSet",
          function(object,
                   split = FALSE,
                   ROIs_high = NULL,
                   features_high = NULL,
              ...
              if (!("sizefact_fitNBth" %in% varLabels(object))) {
                ...
                ROIs_high <-
                  Biobase::sampleNames(object)[which((
                    quantile(fData(object)[["para"]][, 1],
                             probs = 0.90, na.rm = TRUE) - notes(object)[["threshold"]]
            ...
            posdat <-
              object_high[-which(fDat$CodeClass == "Negative"),]
            ...
            if (is.null(features_high)) {
              ...
              if (any(grepl("WTA", toupper(Biobase::annotation(object))))) {
                ...
                if (any(grepl("scores", fvarLabels(object)))) {
                  if (split == TRUE) {
                    sc1_scores <-
                      fData(object)[-which(fData(object)$Negative), grepl("scores_", fvarLabels(object))]
                    rownames(sc1_scores) <-
                      fData(object)[-which(fData(object)$Negative), "TargetName"]
                    features_high <-
                      apply(sc1_scores, 2, function(x) {
                    ...
                    sc1_scores <-
                      fData(object)[-which(fData(object)$Negative), "scores"]
                    names(sc1_scores) <-
                      fData(object)[-which(fData(object)$Negative), "TargetName"]
                    features_high <-
            ...
            if (isFALSE(split)) {
              ...
              # para0
              Biobase::fData(object)[["para0_norm"]] <- matrix(
                NA,
                nrow = nrow(object),
                ncol = nrow(result$para0),
                dimnames = list(Biobase::featureNames(object), paste0("var", seq_len((
                  nrow(result$para0)
                ))))
              )
              Biobase::fData(object)[["para0_norm"]][colnames(result$para0),] <-
                t(result$para0)
              
              # para
              Biobase::fData(object)[["para_norm"]] <- matrix(
                NA,
                nrow = nrow(object),
                ncol = nrow(result$para),
                dimnames = list(Biobase::featureNames(object), paste0("var", seq_len((
                  nrow(result$para)
                ))))
              )
              Biobase::fData(object)[["para_norm"]][colnames(result$para),] <-
                t(result$para)
              ...
              normmat0_mat[colnames(result$normmat0), ROIs_high] <-
                t(result$normmat0)
              Biobase::assayDataElement(object, "normmat0") <-
                normmat0_mat
              ...
              normmat_mat[colnames(result$normmat), ROIs_high] <-
                t(result$normmat)
              Biobase::assayDataElement(object, "normmat") <-
                normmat_mat
              
              # sizefact
              Biobase::pData(object)[ROIs_high, "sizefact_norm"] <-
                result$sizefact
              # sizefac0
              Biobase::pData(object)[ROIs_high, "sizefact0_norm"] <-
                result$sizefact0
              ...
              # conv0
              Biobase::fData(object)[features_high, "conv0"] <-
                result$conv0
              # conv
              Biobase::fData(object)[features_all, "conv"] <-
                result$conv
              
              # features_high
              Biobase::fData(object)[features_high, "features_high"] <-
                1
              # features_all
              Biobase::fData(object)[features_all, "features_all"] <-
                1
            } else {
              if (is.null(Biobase::notes(object)$fitPoisBG_sp_var)) {
                ...
                id <-
                  Biobase::pData(object)[[idvar]][match(ROIs_high, colnames(object))]
              ...
              # threshold0
              Biobase::fData(object)[["threshold0"]] <- matrix(
                NA,
                nrow = nrow(object),
                ncol = nrow(result$threshold0),
                dimnames = list(
                  Biobase::featureNames(object),
                  rownames(result$threshold0)
                )
              )
              Biobase::fData(object)[["threshold0"]][colnames(result$threshold0),] <-
                t(result$threshold0)
              
              # threshold
              Biobase::fData(object)[["threshold"]] <- matrix(
                NA,
                nrow = nrow(object),
                ncol = nrow(result$threshold),
                dimnames = list(
                  Biobase::featureNames(object),
                  rownames(result$threshold)
                )
              )
              Biobase::fData(object)[["threshold"]][colnames(result$threshold),] <-
                t(result$threshold)
              ...
              normmat0_mat[colnames(result$normmat0), ROIs_high] <-
                t(result$normmat0)
              Biobase::assayDataElement(object, "normmat0_sp") <-
                normmat0_mat
              ...
              normmat_mat[colnames(result$normmat), ROIs_high] <-
                t(result$normmat)
              Biobase::assayDataElement(object, "normmat_sp") <-
                normmat_mat
              
              # sizefact
              Biobase::pData(object)[ROIs_high, "sizefact_norm_sp"] <-
                result$sizefact
              # sizefact0
              Biobase::pData(object)[ROIs_high, "sizefact0_norm_sp"] <-
                result$sizefact0
              ...
              for (index in names(result$conv0)) {
                Biobase::fData(object)[features_high, paste0("conv0_sp_", index)] <-
                  result$conv0[[index]]
              }
              # conv
              for (index in names(result$conv0)) {
                Biobase::fData(object)[features_all, paste0("conv_sp_", index)] <-
                  result$conv[[index]]
              }
              # features_high
              Biobase::fData(object)[features_high, "features_high_sp"] <-
                1
              # features_all
              Biobase::fData(object)[features_all, "features_all_sp"] <-
                1
            }
...
setMethod("fitPoisthNorm", "matrix",
          function(object,
            ...
            for (iter in seq_len(iterations)) {
              if (iter == 1) {
                modfit <-
                  PoisthNorm_paraOptall(
                    ...
                    sizefact,
                    preci1,
                    threshold_mean * probenum,
                    preci2,
                    sizescalebythreshold,
                    calhes & (iter == iterations)
                  )
                Im <- modfit$hes
                Im0 <- Im
                para <- modfit$par
              ...
              if (iter == 1) {
                features_remain <-
                  names(which(colMeans(abs(para[seq_len(n_para), , drop = FALSE])) < cutoff))
                if (prior_type == "equal") {
                  if (covrob) {
                    cov_mat <-
                    ...
                    cov_mat <-
                      cov(t(para[seq_len(n_para), features_remain]), use = "pairwise.complete.obs")
                  }
                  ...
                  preci1 <-
                    t(contrmat) %*% solve(cov_mat) %*% contrmat
                  
                  #   avevec <- rep(1, n_para)/n_para
                  
                  preci1 <-
                    confac * preci1 + (preci1con) * t(contrvec) %*% contrvec
                }
              }
              ...
              if (sizefactrec) {
                ...
                message(sprintf("Iteration = %s, squared error = %s", iter, sum((
                  sizefact - sizefact0
                ) ^ 2)))
...
setGeneric("fitPoisthNorm_sp",
           signature = c("object"),
           function(object, ...)
             standardGeneric("fitPoisthNorm_sp"))

#' @rdname fitPoisthNorm_sp-methods
#' @aliases fitPoisthNorm_sp,matrix-method
setMethod("fitPoisthNorm_sp", "matrix",
          function(object,
                   probenum,
                   features_high,
            ...
            preci1 <- list()
            threshold0 <-
              matrix(NA, length(uniid), length(features_high))
            ...
            for (idname in uniid) {
              threshold0[idname,] <-
                normmod_ls[[idname]]$para0[length(loc[[idname]]) + 1,]
              threshold[idname,] <-
                normmod_ls[[idname]]$para[length(loc[[idname]]) + 1,]
              
              normmat0[loc[[idname]],] <-
                normmod_ls[[idname]]$normmat0
              normmat[loc[[idname]],] <- normmod_ls[[idname]]$normmat
              sizefact[loc[[idname]]] <- normmod_ls[[idname]]$sizefact
              sizefact0[loc[[idname]]] <-
                normmod_ls[[idname]]$sizefact0
