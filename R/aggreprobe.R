...
setGeneric("aggreprobe",
           signature = c("object"),
           function(object, ...)
             standardGeneric("aggreprobe"))

#' @rdname aggreprobe-methods
#' @aliases aggreprobe,NanoStringGeoMxSet-method
setMethod("aggreprobe", "NanoStringGeoMxSet",
          function(object,
                   split,
            ...
            if (GeomxTools::featureType(object) == "Target") {
              stop(
                "GeoMxSet object feature type is already target-level. ",
            ...
            if (use == "score" | use == "both") {
              if (isFALSE(split)) {
                if (!any(c(
                  ...
                  stop(
                    "No sizefact and featfact is found. Please run `fitPoisBG` first. If you run `fitPoisBG` with `groupvar` before, please specify `split = TRUE` for `aggreprobe`."
                  )
                }
                sizefact <-
                  setNames(pDat[["sizefact"]], rownames(pDat))
                featfact <-
                  setNames(fDat[["featfact"]], rownames(fDat))
                ...
                if (!any(c(
                  ...
                  stop(
                    "No sizefact and featfact for multiple slides is found. Please run `fitPoisBG` first with `groupvar`."
                  )
                }
                sizefact <-
                  setNames(pDat[["sizefact_sp"]], rownames(pDat))
                featfact <-
                  fDat[, grep("featfact_", colnames(fDat))]
                colnames(featfact) <-
                  gsub("featfact_", "", colnames(featfact))
                idvar <-
                  Biobase::notes(object)[["fitPoisBG_sp_var"]]
                id <- pDat[[idvar]]
            ...
            targetFeats_nonneg$probenum <-
              result$probenum[match(rownames(targetFeats), names(result$probenum))]
            targetFeats_nonneg$proberemained <-
              cbind(result$remain)[match(rownames(targetFeats), names(result$remain)),]
            ...
            targetFeats <-
              Biobase::AnnotatedDataFrame(
                rbind(targetFeats_nonneg[rownames(targetCounts_nonneg),],
...
setMethod("aggreprobe", "matrix",
          function(object,
            ...
            probemat_ls <-
              lapply(split(probenames, featurenames), function(x)
                object[x,])
            ...
            if (use == "score" | use == "both") {
              ...
              remainscores <-
                lapply(scores_ls, function(x)
                  names(which(abs(x - median(
                    x
                  )) <= 1.96 * 1.25 * mad(x))))
            ...
            if (use == "cor" | use == "both") {
              cor_ls <- lapply(probemat_ls, function(x)
                cor(t(x)))[feature_toagre]
              remaincor <- remain[feature_useall]
              remaincortemp <- remain[feature_toagre]
              
              
              mean_cor_ls <- lapply(names(cor_ls), function(x) {
                apply(cor_ls[[x]][remaincortemp[[x]], remaincortemp[[x]]], 2, function(z)
                  (sum(z) - 1) / (length(z) - 1))
              })
              ...
              remaincortemp <- lapply(mean_cor_ls, function(x) {
                if (all(x > corcutoff))
                  ...
                  y <- setdiff(names(x), names(which.min(x)))
                y
              })
              remaincor <-
                c(remaincor, remaincortemp[names(which(sapply(remaincortemp, length) <= 2))])
              remaincortemp <-
                remaincortemp[names(which(sapply(remaincortemp, length) > 2))]
            ...
            if (use == "both") {
              ...
              remain <-
                mapply(function(x, y)
                  intersect(x, y), remaincor, remainscores)
            } else if (use == "score") {
            ...
            featuremat <-
              t(sapply(remain, function(x)
                colSums(object[x, , drop = FALSE])))
