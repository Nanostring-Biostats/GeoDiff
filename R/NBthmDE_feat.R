fitNBthmDEfeat <-
  function(Y,
    ...
    for (iter in seq_len(iterations)) {
      ...
      U <-
        NBthmDE_uOpt(numeric(rl), X, Z, Y, sizefact0, sizefact, para_fix, preciu, 0)$par
      # print(U)
      ## Metropolis Hasting
      ...
      for (iter_conv in seq_len(5)) {
        ...
        if (useprior) {
          covnll <-
            NBthm_rcovnlliWp(Umat, Lambdati, mapping, nu = nu, diag(thetapri, rl))
        } else {
        ...
        result <- tryCatch(
          ...
          error = function(e) {
            message(
              sprintf(
                "`optim` encountered singularity problems. Re-assigning the initial values (%s/5).",
                iter_conv
              )
            )
            return(NULL)
      ...
      if (!conv) {
        result <-
          NBthmDE_fparaOptfeat(
            ...
            threshold_mean,
            preci2,
            Umat[((seq_len(thin_sq[iter])) * floor(nmh_sq[iter] / thin_sq[iter])),],
            para_fix,
            iter == iterations
          )
      } else {
        result <-
          NBthmDE_fparaOptfeat(X,
                               Z,
                               Y,
                               sizefact0,
                               sizefact,
                               ...
                               threshold_mean,
                               preci2,
                               Umat[((seq_len(thin_sq[iterations])) * floor(nmh_sq[iterations] / thin_sq[iterations])),],
                               para_fix,
                               1)
      }
      ...
      if (conv | (iter == iterations)) {
        ...
        gr0 <- NBthmDE_gradM(Y,
                             ...
                             preci2,
                             threshold_mean)
        grt <- plyr::alply(gr0, 2, function(x)
          x %*% t(x))
        a <-
          array(unlist(grt), c(ncol(X) + 2, ncol(X) + 2, length(grt)))
    ...
    Tem <-
      Sigma[seq_len(temp_size), seq_len(temp_size), drop = FALSE]
    Temupper <- Tem
