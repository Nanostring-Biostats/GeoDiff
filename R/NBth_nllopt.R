...
NBth_paranll <-
  function(Y,
    ...
    loglik_ind <- function(x) {
      ...
      tmp1 <-
        sizefact0 * min(featfact, probenum * threshold) + sizefact * max(featfact - probenum * threshold, 0)
      ...
      # - ((log(phi) - m0)^2)/(2 * (sigma^2)) - log(sigma)-(sum(tmp3))
    }
  }
...
NBth_scalenll <-
  function(Y,
    ...
    loglik_ind <- function(sizefact) {
      ...
      tmp1 <-
        sizefact0 * pmin(featfactvec, probenum * threshold) + sizefact * pmax(featfactvec - probenum * threshold, 0)
...
NBth_thnll <-
  function(Y,
    ...
    loglik_ind <- function(threshold) {
      ...
      tmp3 <- dnbinom(
        x = t(as.matrix(Y)),
        size = matrix(rep(rvec, each = length(sizefact)), ncol = length(rvec)),
        mu = tmp1,
        log = TRUE
      )
...
NBth_paraopt <-
  function(countmat,
    ...
    if (is.null(names(probenum)))
      probenum <- rownames(countmat)
    para <- sapply(rownames(countmat), function(x) {
      fun1 <-
        NBth_paranll(countmat[x,], probenum[x], sizefact, sizefact0, threshold)
      result <-
        tryCatch(
          optim(start, fun1, lower = lower, method = "L-BFGS-B")$par,
          error = function(err)
            c(NA, NA)
        )
    })
