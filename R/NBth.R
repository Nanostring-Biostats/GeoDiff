...
setGeneric("fitNBth",
           signature = c("object"),
           function(object, ...)
             standardGeneric("fitNBth"))

#' @rdname fitNBth-methods
#' @aliases fitNBth,NanoStringGeoMxSet-method
setMethod("fitNBth", "NanoStringGeoMxSet",
          function(object,
                   split = TRUE,
                   features_high = NULL,
                   sizefact_BG = NULL,
            ...
            if (is.null(threshold_start)) {
              message(
                sprintf(
                  "`threshold_start` is missing. The default value is estimated based on fitPoisBG results with %s.",
            ...
            # update para0
            Biobase::notes(object)$para0 <-
              ifelse(is.na(result$para0), "NA", result$para0)
            
            # update para
            Biobase::fData(object)[["para"]] <- matrix(
              NA,
              nrow = nrow(object),
              ncol = 2,
              dimnames = list(featureNames(object), c("signal", "r"))
            )
            Biobase::fData(object)[["para"]][colnames(result$para),] <-
              t(result$para)
            ...
            # update preci1
            Biobase::notes(object)$preci1 <-
              ifelse(is.na(result$preci1), "NA", result$preci1)
            
            # update conv0
            Biobase::notes(object)$conv0 <-
              ifelse(is.na(result$conv0), "NA", result$conv0)
            
            # update conv
            Biobase::notes(object)$conv <-
              ifelse(is.na(result$conv), "NA", result$conv)
            
            # update Im
            Biobase::notes(object)$Im <-
              ifelse(is.na(result$Im), "NA", result$Im)
            
            # update features_high
            Biobase::fData(object)[["feature_high_fitNBth"]] <- 0
            Biobase::fData(object)[["feature_high_fitNBth"]][match(result$features_high,
                                                                   Biobase::featureNames(object),
                                                                   nomatch = 0)] <- 1
            
            # update features_all
            Biobase::notes(object)$features_all <-
              ifelse(is.na(result$features_all), "NA", result$features_all)
            
            # update threshold
            Biobase::notes(object)$threshold <-
              ifelse(is.na(result$threshold), "NA", result$threshold)
...
setMethod("fitNBth", "matrix",
          function(object,
            ...
            for (iter in seq_len(iterations)) {
              para <-
                NBth_paraopt(object[features_high,],
                             probenum[features_high],
                             sizefact,
                             sizefact_BG,
                             threshold,
                             start = start_para)
              ...
              features_NA <-
                features_high[unique(which(is.na(para), arr.ind = TRUE)[, 2])]
              features_remain <- setdiff(features_high, features_NA)
              
              
              for (i in seq_len(length(sizefact))) {
                fun <-
                  NBth_scalenll(object[features_remain, i],
                                probenum[features_remain],
                                t(para[1, features_remain]),
                                t(para[2, features_remain]),
                                sizefact_BG[i],
                                threshold)
                sizefact[i] <-
                  optim(
              ...
              if (!threshold_fix) {
                fun1 <-
                  NBth_thnll(object[features_remain,],
                             probenum[features_remain],
                             sizefact,
                             sizefact_BG,
                             scale_fac * t(para[1, features_remain]),
                             t(para[2, features_remain]))
              ...
              if (sum((sizefact - sizefact0) ^ 2) < tol)
                break
